...
from .providers import CHAT_ACTION_TYPES, LEARNING_MODE_IDS, ModuleChatRequest, ProviderOutputError, ProviderUnavailable, StudyPlanRequest, normalize_checkpoint_feedback, normalize_model_name, provider_for, provider_runtime_status
...
def _validate_manifest(plan: dict, allowed_anchors: set[str]) -> None:
    ...
    if plan["providerMode"] not in {"codex_fixture", "live_openai", "human_review"}:
    ...
    if plan["providerMode"] == "live_openai":
...
class StudyPlanView(APIView):
    ...
    def post(self, request):
        ...
        provider_aliases = {"openai": "openai", "live_openai": "openai", "fixture": "fixture", "codex_fixture": "fixture"}
        if provider_mode not in provider_aliases:
            return _error("provider must be openai or fixture")
        try:
        ...
        return Response({
            "providers": [
                ...
                provider_entry(
                    ...
                    model=normalize_model_name(settings.OPENAI_MODEL, "gpt-5.6-terra-high"),
                ),
