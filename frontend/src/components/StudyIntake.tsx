...
export default function StudyIntake() {
  ...
  const liveProviders = providers.filter((item): item is ProviderStatus & { id: LiveProvider } => item.id === "openai");
  ...
  useEffect(() => {
    getProviderStatus().then((status) => {
      ...
      const preferred = status.providers.find((item): item is ProviderStatus & { id: "openai" } => item.id === "openai" && item.available);
