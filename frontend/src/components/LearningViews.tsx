...
function ProviderFeedbackPanel({ feedback, retryAttempt, busy, nextAction, onRetry }: { feedback: ActivityProviderFeedback; retryAttempt: RetryableAttempt | null; busy: boolean; nextAction: string; onRetry: () => void }) {
  ...
  return <section className={`fos-provider-feedback ${completed ? "completed" : "unavailable"}`} role={completed ? "status" : "alert"}>
    <div><span className="fos-eyebrow">SERVER-SIDE EVALUATION</span><h2>{title}</h2><p>{body}</p><small>{anchors} source anchor{anchors === 1 ? "" : "s"}{feedback.uncertainty ? ` · ${feedback.uncertainty} uncertainty` : ""}</small>{feedback.providerErrorCategory ? <small>Failure category: {feedback.providerErrorCategory}</small> : null}</div>
...
function GoalEvidenceFilter({ goals, value, onChange }: { goals: LearningGoal[]; value: string; onChange: (value: string) => void }) {
  ...
  const selectedLabel = value === "all" ? "All goals" : goals.find((goal) => goal.goalId === value)?.title || "All goals";
  const choose = (nextValue: string) => {
    onChange(nextValue);
    setOpen(false);
  };
  const options = [{ value: "all", label: "All goals" }, ...goals.map((goal) => ({ value: goal.goalId, label: goal.title }))];

  return <div className="fos-evidence-filter" ref={filterRef}>
    <span className="fos-evidence-filter-label">Goal</span>
    <button type="button" className="fos-evidence-filter-trigger" aria-haspopup="listbox" aria-expanded={open} aria-label={`Filter evidence by goal: ${selectedLabel}`} onClick={() => setOpen((current) => !current)} onKeyDown={(event) => {
      if (event.key === "Escape") setOpen(false);
      if (event.key === "ArrowDown") { event.preventDefault(); setOpen(true); }
    }}>
      <span>{selectedLabel}</span><FeynmanIcon name="arrow" size={13} />
    </button>
    {open ? <div className="fos-evidence-filter-menu" role="listbox" aria-label="Filter evidence by goal">
      {options.map((option) => <button key={option.value} type="button" role="option" aria-selected={value === option.value} className="fos-evidence-filter-option" onClick={() => choose(option.value)}>{option.label}</button>)}
    </div> : null}
  </div>;
}
