...
export type StudyPlanResponse = {
  ...
  providerMode: "codex_fixture" | "live_openai" | "human_review";
...
export type StudyRemediationVideo = { mode: "openai_slides"; title: string; requestedDurationSeconds: number; actualDurationSeconds: number; providerId: string; voiceProviderId?: string | null; slides: StudyRemediationSlide[] } | { mode: "sequenced_clips"; title: string; requestedDurationSeconds: number; actualDurationSeconds: number; providerId: string; clips: StudyRemediationVideoClip[] };
...
export type StudyRemediationVideoConfig = { mode: "openai_slides" | "sequenced_clips"; provider: string; label: string; configured: boolean; voiceConfigured: boolean; minDurationSeconds: number; maxDurationSeconds: number };
...
export type StudyChatResponse = { state: "answered" | "abstained" | "needs_human_review" | "action_only"; reply: string; reasonCode?: string | null; sourceAnchorIds: string[]; action: StudyChatAction; providerMode: "codex_fixture" | "live_openai" | "human_review"; sourcePackVersion: string; recordVersion: number; reviewRequired?: boolean };

export type ProviderStatus = { id: "openai" | "fixture" | "mistral"; label: string; available: boolean; model: string };
...
export async function generateStudyPlan(input: { subjectId: string; subjectTitle?: string; moduleId?: string; sourceIds: string[]; pastQuestionSourceIds?: string[]; chapterSelection: "chapter_1" | "all"; provider?: "openai" | "fixture"; learningGoal?: "course" | "skill" | "interview" | "viva"; assessmentFocus?: "mastery" | "mock_test" | "conversation" | "viva"; skillLevel?: "beginner" | "intermediate" | "advanced"; goalBrief?: string }): Promise<StudyPlanResponse> {
